...
async def startup_event():
    ...
    # Do NOT start polling here; worker.py handles polling in a separate process
    global bot_app
    if not TELEGRAM_TOKEN:
        logger.warning("TELEGRAM_TOKEN not set; bot will not start.")
        return

    logger.info("FastAPI startup complete. Bot polling is handled by worker process.")
